...
static void print_expand_input(int type) {
	...
	printf("<tr><td align=left class='reportSelectSubTitle'>仅显示%s:</td></tr>\n", seldesc);
...
int main(void) {
	...
	display_info_table("Nagios配置信息", FALSE, &current_authdata);
	printf("</td>\n");
...
void display_hosts(void) {
	...
	printf("<P><DIV ALIGN=CENTER CLASS='dataTitle'>主机%s%s</DIV></P>\n",
	       (*to_expand == '\0' ? " " : " "), (*to_expand == '\0' ? "" : html_encode(to_expand, FALSE)));
	...
	printf("<TH CLASS='data'>重要性 (主机)</TH>");
	printf("<TH CLASS='data'>重要性 (主机 + 服务)</TH>");
	...
	printf("<TH CLASS='data'>事件处理程序</TH>");
	printf("<TH CLASS='data'>启用事件处理程序</TH>");
	printf("<TH CLASS='data'>粘附选项</TH>\n");
	printf("<TH CLASS='data'>启用抖动监测</TH>");
	...
	for(temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) if(((*to_expand) == '\0') || !strcmp(to_expand, temp_host->name)) {
			...
			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_host->obsess == TRUE) ? "是" : "否");

			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_host->checks_enabled == TRUE) ? "是" : "否");

			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_host->accept_passive_checks == TRUE) ? "是" : "否");
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_host->event_handler_enabled == TRUE) ? "是" : "否");
			printf("</TD>\n");
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_host->flap_detection_enabled == TRUE) ? "是" : "否");
			printf("</TD>\n");
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_host->process_performance_data == TRUE) ? "是" : "否");
			printf("</TD>\n");
...
void display_contacts(void) {
	...
	printf("<TH CLASS='data'>Email地址</TH>");
	printf("<TH CLASS='data'>通信地址</TH>");
	printf("<TH CLASS='data'>最小重要性</TH>");
...
void display_services(void) {
	...
	printf("<TH CLASS='data'>描述</TH>\n");
	printf("<TH CLASS='data'>重要性</TH>\n");
	printf("<TH CLASS='data'>最大检查次数</TH>\n");
	printf("<TH CLASS='data'>常规的检查间隔</TH>\n");
	printf("<TH CLASS='data'>检查重试间隔</TH>\n");
	...
	for(temp_service = service_list; temp_service != NULL; temp_service = temp_service->next)
		if(((*to_expand) == '\0') || (!strcmp(to_expand, temp_service->host_name)) || (!strcmp(to_expand, temp_service->description))) {
			...
			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_service->is_volatile == TRUE) ? "是" : "否");

			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_service->obsess == TRUE) ? "是" : "否");

			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_service->checks_enabled == TRUE) ? "是" : "否");

			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_service->accept_passive_checks == TRUE) ? "是" : "否");
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_service->notifications_enabled == TRUE) ? "是" : "否");
			printf("</TD>\n");

			get_interval_time_string(temp_service->notification_interval, time_string, sizeof(time_string));
			printf("<TD CLASS='%s'>%s</TD>\n", bg_class, (temp_service->notification_interval == 0) ? "<i>不重新通知</i>" : html_encode(time_string, FALSE));
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_service->event_handler_enabled == TRUE) ? "是" : "否");
			printf("</TD>\n");
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_service->flap_detection_enabled == TRUE) ? "是" : "否");
			printf("</TD>\n");
			...
			printf("<TD CLASS='%s'>", bg_class);
			printf("%s\n", (temp_service->process_performance_data == TRUE) ? "是" : "否");
			printf("</TD>\n");
...
void display_timeperiods(void) {
	...
	const char *days[7] = {"周日", "周一", "周二", "周三", "周四", "周五", "周六"};
	int odd = 0;
	int day = 0;
	int x = 0;
	const char *bg_class = "";
	...
	printf("<P><DIV ALIGN=CENTER CLASS='dataTitle'>时间周期%s%s</DIV></P>\n",
	       (*to_expand == '\0' ? " " : " "), (*to_expand == '\0' ? "" : html_encode(to_expand, FALSE)));
...
static void display_servicedependency(servicedependency *temp_sd)
{
	...
	printf("<TD CLASS='%s'>%s</TD>", bg_class, (temp_sd->dependency_type == NOTIFICATION_DEPENDENCY) ? "通知" : "执行检测");
...
static void display_hostdependency(hostdependency *temp_hd)
{
	...
	printf("<TD CLASS='%s'>%s</TD>", bg_class, (temp_hd->dependency_type == NOTIFICATION_DEPENDENCY) ? "通知" : "执行检测");
...
void unauthorized_message(void) {
	...
	printf("<P><DIV CLASS='errorMessage'>看起来像是你没有权限查看您所请求的配置信息...</DIV></P>\n");
...
void display_command_expansion(void) {
	...
	printf("<P><DIV ALIGN=CENTER CLASS='dataTitle'>命令展开</DIV></P>\n");
	...
	if((*to_expand) != '\0') {
		...
		printf("<TR CLASS='dataEven'><TD CLASS='dataEven'>展开:</TD><TD CLASS='dataEven'>%s", escape_string(command_args[0]));
			...
			if(!strcmp(temp_command->name, command_args[0])) {
				...
				for(c = commandline; c && (cc = strstr(c, "$"));) {
					...
					if((*cc) == '$') {
						...
						if(!c) printf("<FONT COLOR='#FF0000'> (非正常终止)</FONT>");
						}
					else {
						/* $ARGn$ macro */
						...
						if(((*c) == '\0') || ((*c) == '$')) {
							...
							if((i > 0) && (i <= MAX_COMMAND_ARGUMENTS)) {
								...
								else printf("<FONT COLOR='#0000FF'>(未定义)</FONT>");
								}
							else printf("<FONT COLOR='#FF0000'>(无效 $ARGn$ index: %u)</FONT>", i);
							if((*c) != '\0') c++;
							else printf("<FONT COLOR='#FF0000'> (非正常终止)</FONT>");
							}
						else {
							/* Syntax err in index */
							c = strstr(cc, "$");
				...
				for(i = 1; (i < MAX_COMMAND_ARGUMENTS) && (command_args[i]); i++) {
					...
					if((lead_space[i] > 0) || (trail_space[i] > 0)) {
						printf("<TR CLASS='%s'><TD CLASS='%s' ALIGN='right'><FONT COLOR='#0000FF'>写入空白:</FONT></TD>\n", bg_class, bg_class);
	...
	printf("<TR CLASS='dataEven'><TD><BR/></TD><TD CLASS='dataEven'>从主机或服务配置command_check命令定义, 点击“提交”查看命令扩展</TD></TR>\n");
	printf("<TR CLASS='dataEven'><TD CLASS='dataEven'>展开:</TD><TD CLASS='dataEven'><FORM\n");
	...
	printf("<INPUT TYPE='SUBMIT' VALUE='提交'></FORM></TD></TR>\n");
